  ...
  function showWaitingOverlay(reqId, btn) {
    ...
    const poll = setInterval(() => {
      ...
      if (!rec || rec.status !== 'pending') {
        ...
        if (rec && rec.status === 'approved' && btn && document.body.contains(btn)) {
          window.__wmApproved = true;
          try { btn.click(); } finally { setTimeout(() => { window.__wmApproved = false; }, 50); }
          ...
          toast('Delete rejected by Website Manager');
        }
      }
